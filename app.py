...
# AI 통합 분석
# ═══════════════════════════════════════
SYSTEM_PROMPT = """당신은 교통사고 분석 전문가입니다. 반드시 아래 JSON만 출력하세요.
...
규칙: chartCode는 반드시 위 목록에서 선택. laws 2~3개, cases 1~2개. ratio합=100. 이미지가 있으면 이미지 내용도 분석에 포함. 실제 법규기반. 한국어."""
...
def analyze():
    ...
    if detailed:
        ...
        if duration > 40:
            cap.release()
            filepath.unlink(missing_ok=True)
            return jsonify({"error": "영상은 40초 이하만 분석 가능합니다."}), 400
